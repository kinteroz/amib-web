...
BEGIN
  ...
  IF angel_id IS NULL THEN
    ...
    RAISE NOTICE 'Usuario Ángel Arellano creado con ID: %', angel_id;
  ELSE
    ...
    WHERE id = angel_id;
    RAISE NOTICE 'Usuario Ángel Arellano ya existía. ID: %', angel_id;
  END IF;
END $$;


-- ── PASO 2: Crear el Comité Maestro ──────────────────────────
...
BEGIN
  ...
  -- (Se agrega al propio coordinador como secretario del comité)
